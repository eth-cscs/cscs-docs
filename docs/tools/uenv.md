...
CSCS provides a build service for uenv that takes as its input a uenv recipe, and builds the uenv using the same pipeline that is used by CSCS to build the officially supported uenv.

A uenv recipe is a description of the software to build in the uenv.
See the [stackinator documentation](https://eth-cscs.github.io/stackinator/recipes/) for detailed documentation.
...
!!! warning
    **If, for whatever reason, make a uenv publicly available, do not use the build service.**
