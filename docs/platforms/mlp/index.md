...
Two scratch file systems are mounted, on hardware tuned for different access patterns; choosing the right one matters for performance, as explained under [file system suitability][ref-mlp-storage-suitability].
...
!!! under-construction "Persistent fast storage for project datasets"
    Some projects need large shared datasets to remain on the fast Iopsstor file system without being subject to the scratch cleanup policy.
    A dedicated, persistent project area on Iopsstor is being designed to address this use case.
    Until it is available, keep such datasets on the [project store][ref-storage-store] and stage them to scratch when needed; if this affects your workflow, discuss your requirements with your PI.
