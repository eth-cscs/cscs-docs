...
In particular, this tutorial targets the Beverin cluster, but it should be possible to adapt it with minor changes to other clusters as well.
...
Now we can pull the docker image in our container images directory just created. The following command allows to import a docker image as a squashfs archive that can be used with the [Container Engine][ref-container-engine].
