...
The uenv tool stores images in a repository on each user's scratch, `$SCRATCH/.uenv-images`, which will have to migrate to ritom.

!!! warning "do not use ritom for storing uenv"
...
!!! example 'check whether you are affected'
    Version v9.2.0 of uenv has a fix for the issue.
    ...
    # all good: 9.2.0 and later are fixed
    $ uenv --version
    9.2.0
    ```
...
!!! example 'checking whether there is a repository on Capstor'
    ...
    If the repository does not exist you will see the following output
    ```console
...
!!! example 'new users'
    ...
    then use the `uenv repo status` to check the location of the default repository, which should be in `/capstor/scratch`:
