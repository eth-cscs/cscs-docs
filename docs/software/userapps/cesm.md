...
!!! warning
    Version `26.2` is incompatible with [Eiger][ref-cluster-eiger] after the system update on 2026-08-26.

    Update your workflow to use `esmf/26.7:v1`, after downloading it.

=== "26.7"

    The `26.7` version is a minor update to `26.2`, which provides the same versions of packages as `26.2`.
    It adds fixes to the views that fix runtime linking problems that were triggered after a system upgrade.

    The 26.7 version provides two views for building CESM with and without MPI.
...
```console title="find and download the esmf uenv"
$ uenv image find esmf
uenv          arch  system  id                size(MB)  date
esmf/26.7:v1  zen2  eiger   24b3e37492ce6371     380    2026-08-25
$ uenv image pull esmf/26.7:v1
```
...
!!! example "starting the esmf uenv with the `esmf` view for distributed simulations"
    ...
    ```console
    $ uenv start --view=esmf esmf/26.7:v1
...
!!! example "building CESM `cesm3_0_beta_7`"
    ...
    !!! note
        This workflow works for the `FHIST` compset, and will need to be changed to support your use case.

!!! warning
    The uenv must be used when SLURM jobs are launched, which is done using `srun` flags.

    The flags to pass `srun --uenv=esmf/26.7:v1 --view=esmf` (or `--view=esmf-serial`).
